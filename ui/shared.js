...
const GLOSSARY = {
  ...
  'odds': ['Draft odds', 'The chance you draft every piece in one draft, shown as a headline plus its range. Two things have to happen: the card has to be opened at the table at all (fixed by pod size and pack count), and it has to survive the drafters between its opener and your seat. That second part depends on how much everyone else wants it, so the number is given three ways \u2014 if nobody else is in your lane, at a normal amount of competition, and if everyone first-picks it.<span class="warn">Pieces are treated as independent, which slightly overstates it — they compete for your own picks and for pack space. Read it as an upper bound.</span>'],
  'p(draft': ['Chance of drafting enough', 'The chance you draft at least the number of cards the tactic needs, shown with its range. A broad archetype holds up even when contested; a thin one only works when nobody else is in your lane, which is what the spread tells you.'],
