...
def cube_opportunities(con, cube_id, contention=0.35, players=8, pack_size=15, rounds=3):
    ...
    for name, pair in GUILDS:
        ...
        # Depth times how the lane has actually performed here. It used to be
        # depth times EDHREC play rate, which ranked these ten lanes almost
        # exactly backwards against this table's own results (Spearman -0.32):
        # Boros came last on play rate and third on games won. Popularity in
        # multiplayer Commander is not a claim about a Pioneer cube.
        rate = (seen or {}).get("score_pct")
        opportunity = len(nonland) * (rate / 50.0) if rate else float(len(nonland))
...
def pick_scores(con, cube_id):
    ...
    # the cards the rest of the cube most wants to sit beside this one, which is
    # what makes a first pick a plan rather than a card
    partners = {}
    for pr in syn["pairs"]:
        a, b = pr["cards"][0], pr["cards"][1]
        for me, them in ((a, b), (b, a)):
            partners.setdefault(me["oracle_id"], []).append({
                "oracle_id": them["oracle_id"], "name": them["name"],
                "lift": pr["lift"], "played_together": pr["played_together"],
                "kind": pr["kind"], "hint": pr.get("hint") or "",
            })
    for lst in partners.values():
        # a huge lift on a handful of decks is an anecdote; rank by lift but keep
        # the deck count visible so the reader can discount it
